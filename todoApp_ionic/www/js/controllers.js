...
/*     $scope.loginData ={
  ...
  .controller('StartTaskCtrl', function ($scope, $rootScope, Customer, $state) {
    ...
    $scope.$on('timer-tick', function (event, args) {
      ...
      if ($scope.isTakingABreak)
      {
        var time = +$rootScope.currentTask.breaktime;
        time = time + 1;
        $rootScope.currentTask.breaktime = time;
      }
      else
      {
        var time = +$rootScope.currentTask.worktime;
        time = time + 1;
        $rootScope.currentTask.worktime = time;
      }
    });
